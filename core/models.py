...
# ... (UserManager and User class - Keep as is) ...
class UserManager(BaseUserManager):
    def create_user(self, email, user_type, password=None):
        ...
        if user_type not in ['PATIENT', 'DOCTOR']: raise ValueError("Invalid user type")
...
class DoctorProfile(models.Model):
    ...
    work_hour_start = models.IntegerField(default=9)
    work_hour_end = models.IntegerField(default=17)
    hospital_name = models.CharField(max_length=255, blank=True, null=True)
    hospital_reception_number = models.CharField(max_length=15, blank=True, null=True)
    emergency_contact_number = models.CharField(max_length=15, blank=True, null=True)

    def get_degree_upload_path(instance, filename): return f'doctors/{instance.user.email}/degree_{filename}'
    def get_reg_upload_path(instance, filename): return f'doctors/{instance.user.email}/registration_{filename}'
    def get_photo_upload_path(instance, filename): return f'doctors/{instance.user.email}/photo_{filename}'
    ...
    def __str__(self): return f"Dr. {self.name} ({self.verification_status})"
...
class MedicalReport(models.Model):
    ...
    shared_with = models.ManyToManyField(User, related_name='shared_reports', blank=True, limit_choices_to={'user_type': User.UserType.DOCTOR})
    def __str__(self): return f"Report '{self.title}' for {self.patient.email}"

# ... (DoctorPatientConnection, Appointment, Conversation, Message, PatientHealthMetric - Keep as is) ...
class DoctorPatientConnection(models.Model):
    class ConnectionStatus(models.TextChoices):
        PENDING = 'PENDING', 'Pending'
        ACCEPTED = 'ACCEPTED', 'Accepted'
        REJECTED = 'REJECTED', 'Rejected'
    patient = models.ForeignKey(User, on_delete=models.CASCADE, related_name='doctor_connections', limit_choices_to={'user_type': User.UserType.PATIENT})
    doctor = models.ForeignKey(User, on_delete=models.CASCADE, related_name='patient_connections', limit_choices_to={'user_type': User.UserType.DOCTOR})
    status = models.CharField(max_length=10, choices=ConnectionStatus.choices, default=ConnectionStatus.PENDING)
    created_at = models.DateTimeField(auto_now_add=True)
    class Meta: unique_together = ('patient', 'doctor')
    def __str__(self): return f"{self.patient.email} -> {self.doctor.email} ({self.status})"
...
class Appointment(models.Model):
    ...
    class ConsultationType(models.TextChoices):
        ONLINE = 'ONLINE', 'Online'
        IN_PERSON = 'IN_PERSON', 'In-Person'
    doctor = models.ForeignKey(User, on_delete=models.CASCADE, related_name='doctor_appointments', limit_choices_to={'user_type': User.UserType.DOCTOR})
    patient = models.ForeignKey(User, on_delete=models.SET_NULL, related_name='patient_appointments', limit_choices_to={'user_type': User.UserType.PATIENT}, null=True, blank=True)
    start_time = models.DateTimeField()
    end_time = models.DateTimeField()
    status = models.CharField(max_length=20, choices=AppointmentStatus.choices, default=AppointmentStatus.AVAILABLE)
    consultation_type = models.CharField(max_length=20, choices=ConsultationType.choices, default=ConsultationType.IN_PERSON)
    ...
    def __str__(self):
        if self.patient: return f"Appt for {self.patient.email} with Dr. {self.doctor.doctor_profile.name} at {self.start_time}"
...
class Message(models.Model):
    ...
    def __str__(self): return f"Message from {self.sender.email} at {self.timestamp}"
...
class PatientHealthMetric(models.Model):
    ...
    def __str__(self): return f"Health Metrics for {self.patient.name} ({self.recorded_at.strftime('%Y-%m-%d')})"