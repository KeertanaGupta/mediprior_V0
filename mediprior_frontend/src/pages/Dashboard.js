...
import { Container, Modal, Alert, Row, Col, Card, Button, ListGroup, Badge, Form } from 'react-bootstrap';
...
// --- 1. THIS IS THE FIX: Import VitalsCard, not StatCard ---
...
function Dashboard() {
    const { user, profile, fetchProfile, authTokens } = useAuth(); // <-- Get authTokens
    ...
    const handleVitalsLogged = () => {
        ...
        // This should also re-fetch metrics, let's update PatientDashboard
        if(fetchProfile) fetchProfile(); 
    };

    // --- 2. MOVED HELPER COMPONENTS INSIDE Dashboard ---
    // This gives them access to props like 'onEdit' (handleShowProfileModal)
    ...
    const PatientDashboard = ({ profile, onEdit, onLogVitals }) => {
        ...
        useEffect(() => {
            ...
            fetchLatestMetric();
        }, [authTokens, profile]); // Re-fetch if profile is updated
        ...
        return (
            <Row>
                <Col lg={8}>
                    ...
                    <Row className="mb-4">
                        <Col md={6} lg={3} className="mb-3"><VitalsCard icon={<FiActivity size={24} />} title="Blood Status" value={bloodStatus} unit="mmHg" iconColor="#3a7bff" /></Col>
                        <Col md={6} lg={3} className="mb-3"><VitalsCard icon={<FiHeart size={24} />} title="Heart Rate" value={heartRate} unit="bpm" iconColor="#e63946" /></Col>
                        <Col md={6} lg={3} className="mb-3"><VitalsCard icon={<FiDroplet size={24} />} title="Blood Count" value={bloodCount} unit="Hgb" iconColor="#fca311" /></Col>
                        <Col md={6} lg={3} className="mb-3"><VitalsCard icon={<FiTrendingUp size={24} />} title="Glucose Level" value={glucose} unit="mg/dL" iconColor="#1ee0ac" /></Col>
                    </Row>
                    <Row>
                        <Col md={7} className="mb-4">
                            <HeartRateGraph metrics={metrics} />
                        </Col>
                        <Col md={5}>
                            <BmiCard profile={profile} />
                            <div className="mt-3"><VitalsCard icon={getMoodIcon()} title="Mood" value={mood} unit="" iconColor="#ffc107" /></div>
                            <div className="mt-3"><VitalsCard icon={<FiMoon size={24} />} title="Sleep" value={sleep} unit="hours" iconColor="#6610f2" /></div>
                            <div className="mt-3"><VitalsCard icon={<FiZap size={24} />} title="Steps" value={steps} unit="" iconColor="#fd7e14" /></div>
                        </Col>
                    </Row>
                </Col>
    ...
    // This renders the form (Patient or Doctor) inside the modal
    ...
    // This renders the "Complete Profile" card for new users
        ...
        if (user.user_type === 'DOCTOR') {
            ...
            return (
                <div>
                    ...
                    <Row className="mb-4">
                        {/* --- 3. THIS IS THE FIX: Use VitalsCard --- */}
                        <Col md={4}><VitalsCard title="Total Patients" value="230" detail="3.5% Have increased" icon={<FiUsers />} iconColor="#3a7bff" /></Col>
                        <Col md={4}><VitalsCard title="Surgeries" value="90" detail="Total space ready" icon={<FiClipboard />} iconColor="#1ee0ac" /></Col>
                        <Col md={4}><VitalsCard title="Reviews" value="4.5/5.0" detail="Based on 120 reviews" icon={<FiStar />} iconColor="#ffc107" /></Col>
                    </Row>
                    <Row>
